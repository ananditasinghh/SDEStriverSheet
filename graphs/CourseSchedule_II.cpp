...
//TOPOLOGICAL ALGORITHM 
    ...
    vector<int> findOrder(int N, vector<vector<int>>& P) {
        ...
        for(int i = 0; i < N; i++)
            ...
            // we can start with courses having no prequisites
        
        while(size(q)) {
            auto cur = q.front(); q.pop();
            ans.push_back(cur);                     
            // cur has no remaining pre courses, so we can take it now
            for(auto nextCourse : G[cur]) 
                if(--indegree[nextCourse] == 0)     
                // if there's a next course having 0 prequisite remaining,
                    q.push(nextCourse);             
                    // then we can take it
        }
        if(size(ans) == N) return ans;              
        // ordering exists where all courses can be finished
        return {};                                      
    }
};
